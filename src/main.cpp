      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // Fitting transformed points
          Eigen::VectorXd coeffs = polyfit(waypoints_x, waypoints_y, 3);




          // Courtesy of Mithi (Ex-SDCND Student, and currently mentor)
          //**************************************************************
          //* GET POINTS TO DISPLAY FROM OUR FITTED POLYNOMIAL (ROAD CURVE)
          //**************************************************************
          std::vector<double> next_xs(num_waypoints * 2);
          std::vector<double> next_ys(num_waypoints * 2);
          const double D = 5.0;
          ...
          // double y_des = polyeval(coeffs, px);
          // double psi_des = atan(3 * coeffs[3] * pow(px, 2) + 2 * coeffs[2] * px + coeffs[1]);
          double cte = coeffs[0];
          double epsi = -atan(coeffs[1]);



          // To account for the latency, the states are updated by letting the model run for 100 milliseconds.
          // In Vehicle Coordinates, px, py, and psi are 0

          double dt = 0.1;
          double Lf = 2.67;
          double late_px = v * dt; //cos(0) = 1
          double late_py = 0;    //sin(0) = 0
          double late_psi = 0 - (v / Lf) * delta * dt;
          ...
          std::cout << output[0] << std::setw(10) << output[1] << std::endl;
          std::cout << "________________________" << std::endl;
          ...
          mpc_x_vals = mpc.future_xs;
          mpc_y_vals = mpc.future_ys;
